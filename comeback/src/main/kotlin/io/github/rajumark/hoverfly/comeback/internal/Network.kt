...
/**
 * The Comeback network in plain Kotlin (same math as ComebackExport in comeback/model.py; the
 * same network as Moji):
 *
 *   lexical:  mean of n-gram embeddings -> LayerNorm
 *   semantic: token + position embeddings -> LayerNorm -> 1 transformer layer (4 heads)
 *             -> attention pooling
 *   head:     [lexical, semantic] -> Linear -> GELU -> Linear -> softmax over the reply set
 *
...
internal class Network(bin: InputStream) {
    ...
    init {
        val t = read(bin)
        fun q(n: String) = t[n] as? Q8 ?: error("comeback.bin: missing matrix $n")
        fun f(n: String) = t[n] as? FloatArray ?: error("comeback.bin: missing vector $n")
    ...
    /** Softmax probabilities over the reply set. */
    ...
    companion object {
        ...
        /** Parses comeback.bin (format documented in comeback/scripts/export_kotlin.py; "MOJI" magic, shared format). */
        private fun read(input: InputStream): Map<String, Any> {
            ...
            require(String(magic, Charsets.US_ASCII) == "MOJI") { "not a comeback.bin file" }
            val version = buf.int
            require(version == 1) { "unsupported comeback.bin version $version" }
            ...
            repeat(buf.int) {
                ...
                out[name] = when (dtype) {
                    ...
                    else -> error("comeback.bin: unknown dtype $dtype for $name")
                }
            }
            return out
        }
