...
It uses the same two-stream network as [Moji](https://github.com/rajumark/moji): hashed character n-grams and words on one side, SentencePiece tokens through one transformer layer on the other, then an MLP that scores every reply in the list. It was distilled from a sentence-embedding model and trained on public English dialogue datasets plus synthetic texting data. Weights are int8 with one scale per row.
...
Code: Apache-2.0. The model and reply list were trained on dialogue datasets, some of which are licensed for non-commercial use only; see [NOTICE](NOTICE) before using Comeback in a commercial app.
